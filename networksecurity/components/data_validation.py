...
class DataValidation:
    ...
    def detect_datadrift(self,base_df,current_df,threshold=0.05)->bool:
        try:
            ...
            drift_report_file_path = self.data_validation_artifact.drift_report_file_path
            # Create directory
    ...
    def initiate_data_validate(self)->DataValidationArtifact:
        try:
            ...
            # checking datadrift
            status = self.detect_datadrift(base_df=train_dataframe,current_df=test_dataframe)
            dir_path = os.path.dirname(self.data_validation_artifact.valid_train_file_path)
            os.makedirs(dir_path,exist_ok=True)

            train_dataframe.to_csv(
                self.data_validation_artifact.valid_train_file_path,index=False,header=True
            )
            test_dataframe.to_csv(
                self.data_validation_artifact.valid_test_file_path,index=False,header=True
            )
